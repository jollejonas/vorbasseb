...
function TeamPanel({ t }: { t: TickerTeam }) {
...
export function SportsTickerClient({ teams }: { teams: TickerTeam[] }) {
  ...
  return (
    <div className="sticky top-16 z-40 bg-[#0a0f1e] border-b border-white/10 text-xs text-white px-4 py-1">
      <div className="max-w-6xl mx-auto flex flex-col items-center">

        {/* All team panels stacked in the same grid cell — tallest sets the height */}
        <div className="grid w-full">
          {teams.map((team, i) => (
            <div
              key={i}
              style={{ gridRow: 1, gridColumn: 1 }}
              className={i !== teamIdx ? "invisible" : undefined}
              aria-hidden={i !== teamIdx}
            >
              <TeamPanel t={team} />
            </div>
          ))}
